...
void generate_data(void) {
  ...
  size_t ra1_dec1_length =
      sizeof(size_t) + STARS_PER_SOLID_ANGLE * 3 * sizeof(double);
  size_t ra1_dec180_length = ra1_dec1_length * 180;

  void* buffer = malloc(ra1_dec180_length);
  if (buffer == NULL) goto CLOSE_FILE;

  for (int right_ascension = RIGHT_ASCENSION_MIN;
       right_ascension < RIGHT_ASCENSION_MAX; ++right_ascension) {
    for (int declinaison = DECLINAISON_MIN; declinaison < DECLINAISON_MAX;
         ++declinaison) {
      void* offseted_buffer =
          (char*)buffer + ra1_dec1_length * (declinaison + 90);

      ((size_t*)offseted_buffer)[0] = STARS_PER_SOLID_ANGLE;

      double* coords = (double*)(((char*)offseted_buffer) + sizeof(size_t));

      for (int star_count = 0; star_count < STARS_PER_SOLID_ANGLE * 3;
           star_count += 3) {
        // generate random RA, Dec, Dist
        coords[star_count] = (rand() / (double)RAND_MAX) *
                                 ((right_ascension + 1) - right_ascension) +
                             right_ascension;
        coords[star_count + 1] =
            (rand() / (double)RAND_MAX) * ((declinaison + 1) - declinaison) +
            declinaison;
        coords[star_count + 2] =
            (rand() / (double)RAND_MAX) * (DISTANCE_MAX - DISTANCE_MIN) +
            DISTANCE_MIN;
      }
    }
