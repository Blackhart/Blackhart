...
/**
 * @file BkTransform.h
 * @brief Defines the BkTransform structure and functions for representing and
 * manipulating 3D transformations.
 *
 * This file provides the definition of the BkTransform struct, which represents
 * a position, orientation, and local transformation matrix in 3D space. It
 * includes functions for initialization, manipulation, and querying transform
 * properties.
 */
...
/**
 * @struct BkTransform
 * @brief Structure representing a 3D transformation (position, orientation, and
 * local matrix).
 *
 * The transform combines a position (BkPoint3), an orientation (BkQuaternion),
 * and a local transformation matrix (BkMatrix4x4). The dirty flag indicates
 * whether the local matrix needs to be recalculated.
 */
struct BkTransform {
  struct BkPoint3 position; /**< The position of the transform in 3D space. */
  struct BkQuaternion
      orientation; /**< The orientation of the transform as a quaternion. */
 ...
 *
 * Sets the position to zero, orientation to identity quaternion, and marks the
 * matrix as dirty. Must be called before using the transform.
 *
 ...
 *
 * The transform's position is set to the 'from' point, and its orientation is
 * calculated to face the 'to' point using the specified up vector.
 *
 ...
 */
extern BK_API void BkTransform_LookAt(struct BkTransform* obj,
                                      struct BkPoint3 const* from,
                                      struct BkPoint3 const* to,
                                      struct BkVector3 const* up);
 ...
 */
extern BK_API void BkTransform_Translate_BkVector3(
    struct BkTransform* obj, struct BkVector3 const* translation);
 ...
 */
extern BK_API void BkTransform_Rotate_BkQuaternion(
    struct BkTransform* obj, struct BkQuaternion const* rotation);
 ...
 */
extern BK_API void BkTransform_Rotate_EulerAngle(
    struct BkTransform* obj, struct BkEulerAngles const* euler);
 ...
 */
extern BK_API void BkTransform_Rotate_AngleAxis(
    struct BkTransform* obj, struct BkAngleAxis const* angle_axis);

/**
 * @brief Gets the forward direction vector of the transform.
 *
 * The forward vector is the direction the transform is facing (typically -Z
 * axis in local space).
 *
 * @param obj Pointer to the BkTransform object.
 ...
 *
 * The right vector is perpendicular to both forward and up vectors (typically
 ...
 *
 * The up vector points upward relative to the transform's orientation
 * (typically +Y axis in local space).
 *
 * @param obj Pointer to the BkTransform object.
 ...
 */
extern BK_API void BkTransform_SetPosition(struct BkTransform* obj,
 ...
 */
extern BK_API void BkTransform_SetOrientation(
    struct BkTransform* obj, struct BkQuaternion const* orientation);
