 ...
 *
 * This file provides the definition of the BkPoint3 struct and functions for working with
 * 3D points in Cartesian space, including translation, distance calculation, and interpolation.
 */
 ...
 *
 * A point represents a position in 3D space. It has three components: x, y, and z
 * along the respective axes. Unlike vectors, points are affected by translations.
 */
struct BkPoint3
{
 ...
 */
extern BK_API void		BkPoint3_Assign(struct BkPoint3* __restrict obj, struct BkPoint3 const* __restrict src);
 ...
 * This returns a vector that represents the displacement from point b to point a.
 *
 ...
 *
 * Calculates the straight-line distance: distance = sqrt((a.x-b.x)² + (a.y-b.y)² + (a.z-b.z)²)
 *
...
/**
 * @brief Computes a point rotated around a center point by yaw and pitch angles.
 *
 * Calculates the position of a point that is located at a specified radius from the center
 * and rotated by the given yaw (horizontal) and pitch (vertical) angles.
 *
