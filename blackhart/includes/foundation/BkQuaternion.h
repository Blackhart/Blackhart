...
/**
 * @file BkQuaternion.h
 * @brief Defines the BkQuaternion structure and functions for quaternion operations.
 *
 * This file provides the definition of the BkQuaternion struct and comprehensive quaternion
 * operations including construction, multiplication, normalization, conjugation, inversion,
 * and conversions from other rotation representations.
 */
 ...
 *
 * Quaternions are used to represent rotations in 3D space efficiently and avoid gimbal lock.
 * The quaternion has four components: w (scalar) and x, y, z (vector part).
 */
struct BkQuaternion
{
 ...
 *
 * Converts a rotation represented as an angle (in degrees) around a normalized axis
 * into a quaternion.
 *
 ...
 *
 * Modifies the quaternion to have unit length (magnitude = 1.0) while preserving
 * its rotation. If the quaternion has zero magnitude, the result is undefined.
 *
 ...
 *
 * Creates a new normalized quaternion (unit length) without modifying the original.
 * If the source quaternion has zero magnitude, the result is undefined.
 *
 ...
 *
 * Creates a new quaternion with all components negated without modifying the original.
 *
 ...
 *
 * The inverse of a quaternion is its conjugate divided by its magnitude squared.
 ...
 *
 * This represents the rotation needed to transform from q1's orientation to q2's orientation.
 *
 ...
 *
 * The dot product is computed as: dot = q1.w * q2.w + q1.x * q2.x + q1.y * q2.y + q1.z * q2.z
 *
