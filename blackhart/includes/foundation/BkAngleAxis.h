...
/**
 * @file BkAngleAxis.h
 * @brief Defines the BkAngleAxis structure and functions for representing and manipulating 3D rotations using the angle-axis representation.
 *
 * This file provides the definition of the BkAngleAxis struct, which stores a rotation as an angle (in degrees)
 * and a normalized axis of rotation (as a 3D vector). It also declares a set of functions for constructing and
 * modifying BkAngleAxis objects.
 */
...
/**
 * @struct BkAngleAxis
 * @brief Structure representing a rotation in 3D space using the angle-axis representation.
 *
 * The rotation is expressed as a rotation of `angle` degrees around the specified `axis` (which must be normalized).
 */
struct BkAngleAxis
{
