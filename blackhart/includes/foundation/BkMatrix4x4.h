...
/**
 * @file BkMatrix4x4.h
 * @brief Defines the BkMatrix4x4 structure and functions for 4x4 matrix operations.
 *
 * This file provides the definition of the BkMatrix4x4 struct and comprehensive matrix operations
 * including multiplication, transformations, rotations, scaling, translation, and matrix inversion.
 */
 ...
 *
 * The matrix is stored in column-major order, which is the standard format for OpenGL.
 * The matrix elements are accessed as m[row][column], where m11 is row 1, column 1.
 */
struct BkMatrix4x4
{
    real m11; real m12; real m13; real m14; /**< First row of the matrix. */
    real m21; real m22; real m23; real m24; /**< Second row of the matrix. */
    real m31; real m32; real m33; real m34; /**< Third row of the matrix. */
    real m41; real m42; real m43; real m44; /**< Fourth row of the matrix. */
};
 ...
 * The point is treated as a column vector with an implicit homogeneous w component of 1.0,
 * meaning translations will be applied to the point.
 *
 * @param m Pointer to the 4x4 matrix.
...
/**
 * @brief Multiplies a 4x4 matrix with a column-order point4 (homogeneous coordinates).
 *
 * Performs matrix-point multiplication: result = m * p.
 * The point is treated as a homogeneous coordinate with an explicit w component of 1.0.
 * This allows full 4D transformations including perspective projections.
 *
 * @param m Pointer to the 4x4 matrix.
 ...
 *
 * Converts a quaternion rotation to a 4x4 rotation matrix in column-major order.
 *
 ...
 *
 * Creates a rotation matrix that rotates around an arbitrary axis by a specified angle.
 *
 * @param angle_axis Pointer to the angle-axis structure containing the rotation angle (in degrees)
 *                   and the normalized axis vector.
 * @return The 4x4 rotation matrix.
 */
 ...
 *
 * Creates a rotation matrix from Euler angles in degrees. The rotation order is typically
 * ZYX (yaw-pitch-roll) or XYZ depending on the implementation.
 *
 * @param euler Pointer to the Euler angles structure containing x, y, z rotation angles (in degrees).
 * @return The 4x4 rotation matrix.
 */
 ...
 */
extern BK_API void	BkMatrix4x4_Assign(struct BkMatrix4x4* __restrict obj, struct BkMatrix4x4 const* __restrict src);

/**
 * @brief Transposes a 4x4 matrix.
 *
 * Transposing a matrix swaps its rows and columns. A column-major matrix becomes
 * a row-major matrix and vice versa. For rotation matrices, the transpose equals the inverse.
 *
 ...
 *
 * @param k The uniform scaling coefficient. Values > 1.0 scale up, values < 1.0 scale down.
 ...
 *
 * Creates a translation matrix that translates by the specified amounts along each axis.
 *
 ...
 *
 * Creates a translation matrix that translates by the amounts specified in the vector.
 *
 ...
 *
 * Creates a translation matrix that translates to the position specified by the point.
 *
 ...
 *
 * The determinant is a scalar value that can be used to determine if a matrix is invertible.
 * A matrix with a determinant of zero is singular and cannot be inverted.
 *
 * @param m Pointer to the 4x4 matrix.
 ...
 *
 * The inverse matrix, when multiplied by the original matrix, yields the identity matrix.
 * The matrix must be invertible (determinant != 0) for this operation to succeed.
 *
