...
/**
 * @file BkArray.h
 * @brief Defines the BkArray structure and functions for dynamic array operations.
 *
 * This file provides the definition of the BkArray struct, a generic dynamic array container
 * that can store elements of any type. It includes macros and functions for initialization,
 * element access, insertion, deletion, and memory management.
 */
 ...
 * @param v The container (BkArray structure).
 * @return The capacity of the container (number of elements that can be stored without reallocation).
 */
 ...
 * @return A reference to the element at the specified position (cast to type t).
 */
 ...
 * @param d The data value to use for initializing new elements if size increases.
 */
 ...
 * It automatically manages memory allocation and reallocation as elements are added.
 */
struct BkArray
{
	...
	size_t	capacity;  /**< The allocated capacity (maximum elements without reallocation). */
 ...
 *
 * Prepares the array for use. Must be called before using any other BkArray functions.
 * The array starts empty with no allocated capacity.
 *
...
/**
 * @brief Requests that the BkArray capacity be at least enough to contain n elements.
 *
 * If the requested capacity is greater than the current capacity, the array is reallocated
 * to the new capacity. Otherwise, no action is taken.
 *
...
/**
 * @brief Removes all elements from the BkArray, leaving the container with a size of 0.
 *
 * The capacity remains unchanged. The data is not freed, allowing for efficient reuse.
 *
 * @param obj Pointer to the BkArray container.
 */
 ...
 *
 * This function should be called when the array is no longer needed to free resources.
 * After calling this function, the array should not be used unless reinitialized.
 *
 ...
 *
 * The element at the specified index is removed, and all subsequent elements are shifted
 * to fill the gap. The size of the array is decreased by one.
 *
 * @param obj Pointer to the BkArray container.
 ...
 * @brief Appends a new element to the end of the container.
 *
 * Adds a copy of the data to the end of the array. The array size is increased by one.
 * If the capacity is insufficient, the array is automatically reallocated.
 *
 * @param obj Pointer to the BkArray container.
 * @param data Pointer to the data to insert (will be copied, data_size bytes will be copied).
 */
 ...
 * The capacity remains unchanged. If the array is empty, this function has no effect.
 *
 * @param obj Pointer to the BkArray container.
 */
 ...
 * @brief Inserts a new element at a given position in the BkArray.
 *
 * Inserts a copy of the data at the specified index. All elements at and after the index
 * are shifted to make room. The array size is increased by one.
 *
 * @param obj Pointer to the BkArray container.
...
/**
 * @brief Resizes the BkArray to contain exactly the specified number of elements.
 *
 * If the new size is larger than the current size, new elements are initialized with
 * the value from data. If smaller, excess elements are removed. The capacity may be
 * adjusted if necessary.
 *
 * @param obj Pointer to the BkArray container.
 * @param size The new size of the container.
 * @param data Pointer to the data value to use for initializing new elements (if size increases).
 */
