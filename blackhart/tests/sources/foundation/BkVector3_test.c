...
void BkVector3_FromXYZ_test(void) {
  struct BkVector3 v =
      BkVector3_FromXYZ(BK_REAL(1), BK_REAL(2.0), BK_REAL(4.5));
...
void BkVector3_Normalized_test(void) {
  ...
  TEST_ASSERT_FLOAT_WITHIN(ERROR_LIMIT, (float)v.x,
                           (float)12.0 / (float)sqrt(233));
  TEST_ASSERT_FLOAT_WITHIN(ERROR_LIMIT, (float)v.y,
                           (float)-5.0 / (float)sqrt(233));
  TEST_ASSERT_FLOAT_WITHIN(ERROR_LIMIT, (float)v.z,
                           (float)8.0 / (float)sqrt(233));

  TEST_ASSERT_FLOAT_WITHIN(ERROR_LIMIT, (float)BkVector3_Magnitude(&v),
                           (float)1.0);
}
...
void BkVector3_Normalize_test(void) {
  ...
  TEST_ASSERT_FLOAT_WITHIN(ERROR_LIMIT, (float)out.x,
                           (float)12.0 / (float)sqrt(233));
  TEST_ASSERT_FLOAT_WITHIN(ERROR_LIMIT, (float)out.y,
                           (float)-5.0 / (float)sqrt(233));
  TEST_ASSERT_FLOAT_WITHIN(ERROR_LIMIT, (float)out.z,
                           (float)8.0 / (float)sqrt(233));

  TEST_ASSERT_FLOAT_WITHIN(ERROR_LIMIT, (float)BkVector3_Magnitude(&out),
                           (float)1.0);
}
