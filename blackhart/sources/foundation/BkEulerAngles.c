...
struct BkEulerAngles BkEulerAngles_FromXYZ(real const x, real const y,
                                           real const z) {
...
struct BkEulerAngles BkEulerAngles_FromBkQuaternion(
  ...
  if (BK_REAL(fabs((double)sp)) > BK_REAL(0.9999)) {
    ...
    out.y = BkMath_DegFromRad(BK_REAL(atan2((double)-q->x * q->z + q->w * q->y,
                                            0.5 - q->y * q->y - q->z * q->z)));
    out.z = BK_REAL(0);
  } else {
    ...
    out.y = BkMath_DegFromRad(BK_REAL(atan2((double)q->x * q->z + q->w * q->y,
                                            0.5 - q->x * q->x - q->y * q->y)));
    out.z = BkMath_DegFromRad(BK_REAL(atan2((double)q->x * q->y + q->w * q->z,
                                            0.5 - q->x * q->x - q->z * q->z)));
  }
...
struct BkEulerAngles BkEulerAngles_FromBkMatrix4x4(
...
void BkEulerAngles_Set(struct BkEulerAngles* obj, real const x, real const y,
                       real const z) {
