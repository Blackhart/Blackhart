...
void BkFileSystem_OpenFlux(BkFlux** flux, char const* __restrict filename,
...
void BkFileSystem_WriteToFlux(BkFlux* flux, char const* str) {
  ...
  BK_FATAL(fprintf(flux, str) < 0 || ferror(flux) != 0,
           ((struct BkErrorInfo){
...
void BkFileSystem_ReadFromPath(char const* path, char** buffer,
                               size_t* buffer_size) {
...
void BkFileSystem_ReadFromFlux(BkFlux* flux, char** buffer,
                               size_t* buffer_size) {
  BK_ASSERT(BK_ISNULL(flux));
  BK_ASSERT(BK_ISNULL(buffer));
  BK_ASSERT(BK_ISNULL(buffer_size));

  BK_FATAL(fseek(flux, 0, SEEK_END) != 0 || ferror(flux) != 0,
           ((struct BkErrorInfo){
...
void BkFileSystem_CombinePath(char* __restrict dst, char const* __restrict str1,
                              char const* __restrict str2) {
  size_t str1_s = 0;
  size_t str2_s = 0;

  if (str1 != NULL) str1_s = strlen(str1);

  if (str2 != NULL) str2_s = strlen(str2);
