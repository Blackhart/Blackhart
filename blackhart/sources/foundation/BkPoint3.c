...
struct BkPoint3 BkPoint3_Zero(void) {
  return (struct BkPoint3){
      .x = BK_REAL(0.0), .y = BK_REAL(0.0), .z = BK_REAL(0.0)};
}

struct BkPoint3 BkPoint3_FromXYZ(real const x, real const y, real const z) {
  return (struct BkPoint3){.x = x, .y = y, .z = z};
}

void BkPoint3_Set(struct BkPoint3* obj, real const x, real const y,
                  real const z) {
...
struct BkPoint3 BkPoint3_Copy(struct BkPoint3 const* src) {
  ...
  return (struct BkPoint3){.x = src->x, .y = src->y, .z = src->z};
}

void BkPoint3_Assign(struct BkPoint3* __restrict obj,
...
struct BkPoint3 BkPoint3_Add_BkVector3(struct BkPoint3 const* p,
...
struct BkPoint3 BkPoint3_Sub_BkVector3(struct BkPoint3 const* p,
                                       struct BkVector3 const* v) {
  BK_ASSERT(BK_ISNULL(p));
  BK_ASSERT(BK_ISNULL(v));
...
struct BkPoint3 BkPoint3_Lerp(struct BkPoint3 const* from,
                              struct BkPoint3 const* to, real const t) {
...
struct BkVector3 BkPoint3_Sub_BkPoint3(struct BkPoint3 const* a,
...
struct BkPoint3 BkPoint3_RotateAround(struct BkPoint3 const* center,
                                      real const radius, real const yaw,
                                      real const pitch) {
  ...
  position.x = center->x + radius * BK_REAL(cos((double)pitch_radian)) *
                               BK_REAL(sin((double)yaw_radian));
  position.y = center->y + radius * BK_REAL(sin((double)pitch_radian));
  position.z = center->z + radius * BK_REAL(cos((double)pitch_radian)) *
                               BK_REAL(cos((double)yaw_radian));
