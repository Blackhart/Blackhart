...
void	BkPoint3_Assign(struct BkPoint3* __restrict obj, struct BkPoint3 const* __restrict src)
{
	BK_ASSERT(BK_ISNULL(obj));
	BK_ASSERT(BK_ISNULL(src));
...
struct BkPoint3	BkPoint3_RotateAround(struct BkPoint3 const* center, real const radius, real const yaw, real const pitch)
{
	...
	position.x = center->x + radius * BK_REAL(cos((double)pitch_radian)) * BK_REAL(sin((double)yaw_radian));
	position.y = center->y + radius * BK_REAL(sin((double)pitch_radian));
	position.z = center->z + radius * BK_REAL(cos((double)pitch_radian)) * BK_REAL(cos((double)yaw_radian));
