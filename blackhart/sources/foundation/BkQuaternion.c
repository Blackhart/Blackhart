...
struct BkQuaternion BkQuaternion_Mul_BkQuaternion(
...
struct BkQuaternion BkQuaternion_FromAngleAxis(
  ...
  return (struct BkQuaternion){.w = BK_REAL(cos(rao2)),
...
struct BkQuaternion BkQuaternion_FromBkMatrix4x4(struct BkMatrix4x4 const* m) {
  ...
  real const biggestVal =
      BK_REAL(sqrt(fourBiggestSquaredMinus1 + BK_REAL(1.0)) * BK_REAL(0.5));
...
struct BkQuaternion BkQuaternion_FromEulerAngles(
  ...
  return (struct BkQuaternion){.w = hw * pw * bw + hy * px * bz,
                               .x = hw * px * bw + hy * pw * bz,
                               .y = hy * pw * bw - hw * px * bz,
                               .z = hw * pw * bz - hy * px * bw};
}
...
void BkQuaternion_Set(struct BkQuaternion* obj, real const w, real const x,
...
void BkQuaternion_Normalized(struct BkQuaternion* obj) {
  ...
  real const m = BK_REAL(sqrt(obj->w * obj->w + obj->x * obj->x +
...
struct BkQuaternion BkQuaternion_Normalize(struct BkQuaternion const* q) {
  ...
  real const m =
      BK_REAL(sqrt(q->w * q->w + q->x * q->x + q->y * q->y + q->z * q->z));
...
struct BkQuaternion BkQuaternion_Difference(struct BkQuaternion const* q1,
...
real BkQuaternion_Dot(struct BkQuaternion const* q1,
                      struct BkQuaternion const* q2) {
  BK_ASSERT(BK_ISNULL(q1));
  BK_ASSERT(BK_ISNULL(q2));
