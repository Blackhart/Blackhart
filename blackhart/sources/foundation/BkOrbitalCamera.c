...
void BkOrbitalCamera_Rotate(struct BkOrbitalCamera* obj, real const yaw,
                            real const pitch) {
  BK_ASSERT(BK_ISNULL(obj));
  ...
  obj->pitch = BkMath_Clamp(obj->pitch + pitch, obj->pitch_min_limit,
                            obj->pitch_max_limit);
...
void BkOrbitalCamera_SetTarget(struct BkOrbitalCamera* obj,
...
void BkOrbitalCamera_SetPitchBoundLimits(struct BkOrbitalCamera* obj,
                                         real const pitch_min_limit,
...
void _BkOrbitalCamera_RecomputeTransform(struct BkOrbitalCamera* obj) {
  ...
  struct BkPoint3 const position =
      BkPoint3_RotateAround(&(obj->target), obj->radius, obj->yaw, obj->pitch);

  struct BkVector3 const up =
      BkVector3_FromXYZ(BK_REAL(0), BK_REAL(1), BK_REAL(0));
