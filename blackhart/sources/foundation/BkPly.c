...
{
	if (BK_ISNULL(points) || BK_ISNULL(*points))
		return;
...
{
	BK_ERROR(BK_ISNULL(filename), ((struct BkErrorInfo){
	...
	BK_ERROR(BK_ISNULL(ply), ((struct BkErrorInfo){
		.what = "Cannot open PLY file",
		.why = "File does not exist, is not readable, or is not a valid PLY",
		.where = filename,
		.how = "Check the path, file permissions, and that the file starts with 'ply'",
		.result = __BkPly_LoadAborted,
	}), NULL);
	...
	{
		ply_close(ply);
		BK_ERROR(true, ((struct BkErrorInfo){
			.what = "Cannot parse PLY header",
			.why = "Header is missing, incomplete, or malformed (expected 'end_header')",
			.where = filename,
			.how = "Re-export the asset with a complete PLY header ending with 'end_header'",
			.result = __BkPly_LoadAborted,
		}), NULL);
	}
	...
	long const nvertices = ply_set_read_cb(ply, "vertex", "x", __BkPly_VertexCallback, points, 0);
	if (nvertices <= 0
		|| ply_set_read_cb(ply, "vertex", "y", __BkPly_VertexCallback, points, 1) <= 0
		|| ply_set_read_cb(ply, "vertex", "z", __BkPly_VertexCallback, points, 2) <= 0)
	{
		__BkPly_ReleasePoints(&points);
		ply_close(ply);
		BK_ERROR(true, ((struct BkErrorInfo){
			.what = "Cannot find PLY vertex positions",
			.why = "Missing element 'vertex' or properties 'x'/'y'/'z', or vertex count is 0",
			.where = filename,
			.how = "Export with 'element vertex N' (N > 0) and scalar properties x, y, z",
			.result = __BkPly_LoadAborted,
		}), NULL);
	}
	...
	if (ply_read(ply) == 0)
	{
		...
		BK_ERROR(true, ((struct BkErrorInfo){
			...
			.why = "Payload after the header is corrupt or does not match the header",
			.where = filename,
			.how = "Re-export the PLY or verify vertex count and property types match the header",
			.result = __BkPly_LoadAborted,
		}), NULL);
	}
