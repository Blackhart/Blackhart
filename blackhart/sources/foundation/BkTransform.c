...
struct BkMatrix4x4* BkTransform_Matrix(struct BkTransform* obj) {
  ...
  if (obj->dirty) {
    struct BkMatrix4x4 const mt =
        BkMatrix4x4_Translation_BkPoint3(&(obj->position));
    struct BkMatrix4x4 const mr =
        BkMatrix4x4_FromBkQuaternion(&(obj->orientation));
...
void BkTransform_LookAt(struct BkTransform* obj, struct BkPoint3 const* from,
                        struct BkPoint3 const* to, struct BkVector3 const* up) {
  BK_ASSERT(BK_ISNULL(obj));
...
void BkTransform_Translate_BkVector3(struct BkTransform* obj,
                                     struct BkVector3 const* translation) {
  BK_ASSERT(BK_ISNULL(obj));
...
void BkTransform_Rotate_BkQuaternion(struct BkTransform* obj,
                                     struct BkQuaternion const* rotation) {
  BK_ASSERT(BK_ISNULL(obj));
  BK_ASSERT(BK_ISNULL(rotation));

  obj->orientation =
      BkQuaternion_Mul_BkQuaternion(&(obj->orientation), rotation);

  obj->dirty = true;
}

void BkTransform_Rotate_EulerAngle(struct BkTransform* obj,
...
void BkTransform_Rotate_AngleAxis(struct BkTransform* obj,
...
void BkTransform_SetPosition(struct BkTransform* obj,
...
void BkTransform_SetOrientation(struct BkTransform* obj,
